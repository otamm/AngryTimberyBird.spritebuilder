...
class MainScene: CCNode, CCPhysicsCollisionDelegate {
    ...
    let distanceBetweenObstacles:CGFloat = 320; //288; // should be either a multiple or divisor of 256 to avoid a bug.
    ...
    var themeImpulses:[CGFloat] = [10000, 18000, 8000];
    var themeGravities:[Float] = [-2000, -2500, -900];
    var themeFlights:[Float] = [210, 250, 200];
    let xSpeedIncrement:CGFloat = 1.04;
    var xSpeedIncremented:CGFloat = 0;
    let ySpeedIncrement:Float = 1.04; // y speed increment equals xSpeedIncrement * (sin45 of xSpeedIncrement)
    ...
    func didLoadFromCCB() {
        ...
        for i in 0..<4 {
            ...
            let iCG = CGFloat(i);
            let gb = self.groundBlocks[i].contentSize.height;
            self.backgrounds[i].scaleY *= 1.19;
            self.backgrounds[i].position = CGPoint(x: background.contentSize.width * CGFloat(iCG) - iCG, y: gb - 50);
        }
        ...
        for i in 0..<4 {
            ...
            pig.position = CGPoint(x: ((self.distanceBetweenObstacles - 2 * self.obstacleWidth) / 2) + self.nextObstaclePosition + (CGFloat(i) * self.distanceBetweenObstacles) + self.obstacleWidth, y: randomY);
            //println("x: \(pig.position.x), y: \(pig.position.y)");
        }
        ...
        for i in 0..<4 {
            // sets up obstacle max and min Y points.
            self.obstacles[i].setupPositionPoints(self.groundBlocks[0].contentSize.height, screenHeight: CCDirector.sharedDirector().viewSize().height);
            // spawns randomized obstacle.
    ...
    func ccPhysicsCollisionBegin(pair: CCPhysicsCollisionPair!, bird: CCNode!, goal: Goal!) -> ObjCBool {
        ...
        self.birdSpeedX = self.birdSpeedX * self.xSpeedIncrement;
        self.flight = self.flight * self.ySpeedIncrement;
        self.gravity = self.gravity * self.ySpeedIncrement;
        self.flightImpulse = self.flightImpulse * CGFloat(self.ySpeedIncrement);
    ...
    func ccPhysicsCollisionBegin(pair: CCPhysicsCollisionPair!, bird: CCNode!, pig: Pig!) -> ObjCBool {
        if (!pig.isPopped) {
            ...
            if (self.scoreMultiplier <= 5) {
                // max of 300 extra points in the pig-pop streak
    ...
    override func touchBegan(touch: CCTouch!, withEvent event: CCTouchEvent!) {
        if (self.gameStarted) {
            // makes bird go up
            self.bird.physicsBody.applyImpulse(ccp(0, self.flightImpulse));
            ...
            if !(self.isFirstLoad || self.playAnimations) {
                ...
                //iAdHandler.sharedInstance.adBannerView.hidden = false;
                // chances interstitial ad display
                if (CCRANDOM_0_1() > 0.3) {
    ...
    func spawnNewBackground() {
        ...
        self.backgrounds[self.backgroundIndex].position.x += CGFloat(self.backgrounds.count) * -(self.minusBackgroundWidth) - 3;
    ...
    func spawnNewPig() {
        ...
        self.pigs[self.offscreenPigIndex].position = CGPoint(x: self.lastPigPosition + self.distanceBetweenObstacles, y: randomY);
